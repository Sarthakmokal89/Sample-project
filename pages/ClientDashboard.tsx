...
import { Sentiment, Urgency, Feedback, Platform } from '../types';
                ...
                {highUrgencyItems.map(item => (
                    <div key={item.id}>
                        <div className="flex items-center justify-between text-sm">
                            <p className="font-medium text-gray-300 capitalize">{item.platform} - <span className="text-gray-400 font-normal">{item.message.body}</span></p>
                            <span className={urgencyClasses}>{item.urgency}</span>
                        </div>
                        <p className="text-xs text-gray-500 truncate">{item.message.subject}</p>
...
const ClientDashboard: React.FC = () => {
    const hasData = MOCK_FEEDBACK_DATA.length > 0;
    const totalFeedback = MOCK_FEEDBACK_DATA.length;
    const negativeSentiment = MOCK_FEEDBACK_DATA.filter(f => f.sentiment === Sentiment.Negative).length;
    const positiveSentiment = MOCK_FEEDBACK_DATA.filter(f => f.sentiment === Sentiment.Positive).length;
    const highUrgency = MOCK_FEEDBACK_DATA.filter(f => f.urgency === Urgency.High).length;
        ...
        <div className="space-y-6">
            ...
            {hasData ? (
                <>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2">
                            <ChartTrends data={MOCK_FEEDBACK_DATA} />
                        </div>
                        <div className="lg:col-span-1">
                            <ChartSentiment data={MOCK_FEEDBACK_DATA} />
                        </div>
                    </div>

                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <ChartPlatform data={MOCK_FEEDBACK_DATA} />
                        <UrgencyList data={MOCK_FEEDBACK_DATA} />
                    </div>
                </>
            ) : (
                <div className="bg-violet-900 border border-violet-800/80 rounded-xl shadow-lg p-12 text-center mt-6">
                    ...
                    <h3 className="text-xl font-semibold text-white">No Feedback Data</h3>
                    <p className="text-gray-400 mt-2">Connect a data source or add feedback to see your dashboard.</p>
                </div>
            )}
        </div>
    );
